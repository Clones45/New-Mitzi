...
allHtmlFiles.forEach(file => {
    ...
    let faqListHtml = `
                    ...
                    <div class="footer-links faq-footer-links">
                        <a href="${isFaqPage ? '../faq.html' : 'faq.html'}" style="text-decoration:none;"><h4>FAQ Resources</h4></a>
                        <div class="faq-list">`;

    faqs.forEach(faq => {
        let question = faq.question;
        faqListHtml += `\n                            <a href="${rootPath}faq/${faq.slug}.html" title="${question.replace(/"/g, '&quot;')}">${question}</a>`;
    });
    ...
    if (faqBlockRegex.test(content)) {
        let newContent = content.replace(faqBlockRegex, faqListHtml + '\n\n                    ');
...
console.log(`Updated ${updatedFiles} files with the 30-link FAQ block in grid format.`);
